...
import {SET_OBJECTS, SET_SELECTED_OBJECT, setSelectedObject} from "../../store/reducers/mapObjectsReducer";
import {SET_SELECTED_TYPE} from "../../store/reducers/globalReducer";
        ...
        () => {
                setEventsMarkers(eventsInfo.map( (el, index) => (
                    <Marker
                        key={index}
                        position={el.way.coordinates.reverse()}
                        icon={getIcon(16, iconCircleBlue)}
                        eventHandlers={
                            {
                                click: () => {
